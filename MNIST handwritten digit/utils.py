...
def load_mnist_trainval():
    ...
    val_label = label[int(0.8 * len(label)):]
...
def plot_curves(train_loss_history, train_acc_history, valid_loss_history, valid_acc_history):
    ...
    print("Final Accuracy on Training Data: {accuracy:.4f}".format(accuracy=np.max(train_acc_history)))
    print("Final Accuracy on Valid Data: {accuracy:.4f}".format(accuracy=np.max(valid_acc_history)))
