...
class TwoLayerNet(_baseNetwork):
    ...
    def forward(self, X, y, mode='train'):
        ...
        A = self.sigmoid(Z1)
        Z2 = np.dot(A, self.weights['W2']) + self.weights['b2']   # self.sigmoid(Z2)
        p = self.softmax(Z2)
        ...
        L_Z2 = (p - one_hot) / len(y)
        Z2_W2 = A.T
        Z2_A = self.weights['W2'].T
        A_Z1 = self.sigmoid_dev(Z1)
        Z1_W1 = X.T
        self.gradients['b2'] = np.sum(L_Z2, axis=0)
        self.gradients['W2'] = np.dot(Z2_W2, L_Z2)
        self.gradients['b1'] = np.sum(np.dot(L_Z2, Z2_A) * A_Z1, axis=0)
        self.gradients['W1'] = np.dot(Z1_W1, np.dot(L_Z2, Z2_A) * A_Z1)
