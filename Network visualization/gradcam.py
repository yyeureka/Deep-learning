...
visualize_attr_maps('visualization/ggc_gbp_captum.png', X, y, class_names,
                    [attr_ggc, attr_gbp], ['Guided GradCam', 'Guided Backprop'])
...
layer_act_attr_sum = LayerAttribution.interpolate(layer_act_attr_sum, (224, 224)).squeeze()
...
layer_cd = LayerConductance(model, layer)
layer_cd_attr = compute_attributions(layer_cd, X_tensor, target=y_tensor)
layer_cd_attr_sum = layer_cd_attr.mean(axis=1, keepdim=True)
layer_cd_attr_sum = LayerAttribution.interpolate(layer_cd_attr_sum, (224, 224)).squeeze()
...
layer_gc_attr_sum = LayerAttribution.interpolate(layer_gc_attr_sum, (224, 224)).squeeze()

visualize_attr_maps('visualization/layer_captum.png', X, y, class_names,
                    [layer_act_attr_sum, layer_cd_attr_sum, layer_gc_attr_sum],
                    ['Layer Activation', 'Layer Conductance', 'Layer GradCam'],
