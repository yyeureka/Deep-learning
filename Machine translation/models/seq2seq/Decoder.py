...
class Decoder(nn.Module):
    ...
    def __init__(self, emb_size, encoder_hidden_size, decoder_hidden_size, output_size, dropout=0.2, model_type="RNN"):
        ...
        if "RNN" == model_type:
            self.rnn = nn.RNN(emb_size, decoder_hidden_size, dropout=dropout, batch_first=True)
        elif "LSTM" == model_type:
            self.rnn = nn.LSTM(emb_size, decoder_hidden_size, dropout=dropout, batch_first=True)
    ...
    def forward(self, input, hidden):
        ...
        output = output.view(input.shape[1], input.shape[0], -1)
