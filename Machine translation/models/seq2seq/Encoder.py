...
class Encoder(nn.Module):
    ...
    def __init__(self, input_size, emb_size, encoder_hidden_size, decoder_hidden_size, dropout=0.2, model_type="RNN"):
        ...
        if "RNN" == model_type:
            self.rnn = nn.RNN(emb_size, encoder_hidden_size, dropout=dropout, batch_first=True)
        elif "LSTM" == model_type:
            self.rnn = nn.LSTM(emb_size, encoder_hidden_size, dropout=dropout, batch_first=True)
    ...
    def forward(self, input):
        ...
        elif "LSTM" == self.model_type:
            output, (hidden, cell) = self.rnn(embedded)
            hidden = torch.tanh(self.linear2(self.relu(self.linear1(hidden))))
            cell = torch.tanh(self.linear2(self.relu(self.linear1(cell))))
            hidden = (hidden, cell)
